...
export class RaceGameUI extends BaseComponent {
  ...
  private renderGameHeader(): HTMLElement {
    ...
    const timeDisplay = DOMUtils.createElement('div', {
      className:
        'flex flex-col items-center gap-2 text-white h-full justify-end self-end text-3xl md:text-4xl md:self-auto md:justify-center',
    });
  ...
  <div class="flex justify-between items-center mb-8">
    <div class="flex flex-col items-center">
      ...
      ${isRanked ? `<div class="text-sm text-black mb-2">ELO ${user.elo.race}</div>` : ''}
      ${
        data.eloChange
