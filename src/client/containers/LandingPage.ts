...
export class LandingPage extends BaseComponent {
  ...
  private async register(endpoint: string, body: {[k: string]: FormDataEntryValue}, errorMessage: HTMLElement): Promise<void> {
    try {
  ...
  private renderRegistrationForm(mode: 'login' | 'signup'): void {
    ...
    dialogHeader.appendChild(DOMUtils.createElement('i', {
      className: 'w-8 h-8 text-white hover:text-pink-200 hover:animate-spin',
      attributes: {
        icon: 'helloKitty'
      }
    }));
    dialogHeader.appendChild(DOMUtils.createElement('h1', {
    ...
    this.authDialogContent.appendChild(DOMUtils.createElement('p', {
      textContent: signin ? "So wonderful to see you again! Please sign in to continue our pong adventures. 🏓"
    ...
    const googleButton = DOMUtils.createElement('button', {
      ...
      className: 'h-14 w-full rounded-t-2xl border border-white bg-linear-to-br from-background to-background text-white hover:from-pink-200 hover:to-pink-300 hover:text-black hover:border-pink-300 cursor-pointer'
    });
    googleButton.appendChild(DOMUtils.createElement('strong', {
      textContent: ' Google'
    }));
    ...
    const ftButton = DOMUtils.createElement('button', {
      textContent: `Sign ${signin ? "in" : "up"} with`,
      className: 'mt-1 h-14 w-full rounded-b-2xl border border-white bg-linear-to-br from-background to-background text-white hover:from-pink-200 hover:to-pink-300 hover:text-black hover:border-pink-300 cursor-pointer'
    });
    ftButton.appendChild(DOMUtils.createElement('strong', {
      textContent: ' 42'
    }));
    ...
    separator.appendChild(DOMUtils.createElement('span', {
      textContent: 'OR',
    ...
    const handleSubmitForm = (evt: Event) => {
      ...
      const body = Object.fromEntries(new FormData(evt.target as HTMLFormElement).entries());
    ...
    form.appendChild(DOMUtils.createElement('input', {
      className: 'border w-full flex justify-center border-white fill-red-500 focus:outline-none pl-12 py-3 px-5 rounded-lg',
      attributes: {
    ...
    form.appendChild(DOMUtils.createElement('input', {
      className: 'border w-full flex justify-center border-white focus:outline-none pl-12 py-3 px-5 rounded-lg',
      attributes: {
        type: 'password',
    ...
    form.appendChild(DOMUtils.createElement('button', {
      className: 'w-full h-16 rounded-full font-bold uppercase bg-linear-to-br from-pink-200 to-pink-300 text-black mt-6 shadow-lg shadow-pink-300/20 hover:shadow-pink-300/30 hover:-translate-y-1 transition-all',
    ...
    const registrationLink = DOMUtils.createElement('p', {
      ...
      textContent: signin ? "Don't have a pong account yet?" : "Already have a pong account?"
    });
    registrationLink.appendChild(DOMUtils.createElement('span', {
      className: 'text-pink-300 font-bold ml-1 cursor-pointer hover:underline',
      textContent: signin ? "Sign up" : "Log in",
      events: {
        click: (evt) => this.renderRegistrationForm(signin ? 'signup' : 'login')
      }
    }));
  ...
  private renderWelcomeContainer(showAuthDialog: () => void) {
    ...
    text.appendChild(DOMUtils.createElement('span', {
    ...
    text.appendChild(DOMUtils.createElement('span', {
      textContent: ", and I've prepared a special Pong adventure just for you and your friends - play together locally or online, compete in magical tournaments, and chat about all our fun matches!"
    }));
    ...
    buttons.appendChild(DOMUtils.createElement('button', {
      textContent: 'Log in',
      className: 'rounded-full w-50 uppercase font-bold h-full bg-linear-to-br from-pink-200 to-pink-300 text-black shadow-lg shadow-pink-300/20 hover:brightness-95 hover:shadow-pink-300/40 hover:-translate-y-1 transition-all',
      events: {
        click: async (evt) => {
          try {
    ...
    buttons.appendChild(DOMUtils.createElement('button', {
      textContent: 'Sign up',
      className: 'rounded-full w-50 uppercase font-bold h-full bg-gray-500/20 text-pink-300 shadow-lg hover:brightness-95 hover:-translate-y-1 transition-all',
      events: {
        click: (evt) => {
  ...
  private renderKPIBlock = (icon: string, title: string, value: string, darkMode: boolean) => {
    ...
    valueContainer.appendChild(DOMUtils.createElement('p', {
  ...
  render(): HTMLElement {
    ...
    dialogContent.className = 'max-w-[550px] text-white bg-background border border-white rounded-3xl p-16';
